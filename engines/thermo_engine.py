...
class ThermoEngine(CalculationEngine):
    ...
    def get_property(
        ...
        if T <= 0 or T > 2000:
            raise ConditionsOutOfRangeError(
                self.name, f"Temperature {T} K is out of valid range (0 < T <= 2000 K)"
            )
        if P < 0 or P > 1e9:
            raise ConditionsOutOfRangeError(
                self.name, f"Pressure {P} Pa is out of valid range (0 <= P <= 1e9 Pa)"
            )
    ...
    def calculate_bubble_point(
        ...
        try:
            ...
            if not converged:
                logger.warning("Bubble point did not converge for %s at P=%s Pa", substances, pressure)
            return result
    ...
    def calculate_dew_point(
        ...
        try:
            ...
            if not converged:
                logger.warning("Dew point did not converge for %s at P=%s Pa", substances, pressure)
            return result
